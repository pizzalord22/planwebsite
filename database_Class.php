...
class Database
{
    ...
    function projectview($ID){
        $this->select("*","projecten","projectID = ".$ID);
        $query = $this->_result;
        if($query === false){
        ...
        while($row = mysqli_fetch_array($query)){
            ...
            $this->_projectName = $row["projectName"];
            $this->_projectDescription = $row["description"];
        }
    }

    function showProject($where = "true")
    {
        $this->select('*', 'projecten', $where);
...
class insertClass extends Database
{
    ...
    /*
     * insert function
     * the table(s), row(s) and value(s) should be open and the function should be able to replace all existing insert functions/ functionalitie
     *  private function insert($table, $rows,  $values)
     *  {
     *      $this->_sql = "insert into " . $table . " " . $rows . " VALUES "  . $values;
     *      $this->_result = mysqli_query($this->_con, $this->_sql);
     *  }
     */
}
...
class login extends Database
{
    ...
    {
        $username = mysqli_real_escape_string($this->_con, $username);
        $this->select("wachtwoord", "gebruikers", "gebruikersNaam = '" . $username . "'");
        while ($row = mysqli_fetch_array($this->_result)) {
